...
import { Button, Card, FormControl, FormLabel, Input, Select, MenuItem } from "@mui/material";
...
function CarServiceRequestForm({ clientId, carId, serviceId }) {
    ...
    const handleCreate = () => {
        fetch(`http://localhost:8080/api/customers/${formData.clientId}/cars/${formData.carId}/car-services`, {
            method: "POST",
            headers: {
    ...
    const handleUpdate = () => {
        fetch(`http://localhost:8080/api/customers/${formData.clientId}/cars/${formData.carId}/car-services/${formData.id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
